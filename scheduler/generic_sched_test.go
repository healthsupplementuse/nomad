...
func TestServiceSched_BlockedReschedule(t *testing.T) {
	...
	// Process the follow-up eval, which results in no new plan because we want
	// to stop but there's no replacement
	must.NoError(t, h.Process(NewServiceScheduler, followupEval))
	must.Len(t, 3, h.Plans)
	...
	must.MapLen(t, 1, h.Plans[3].NodeAllocation) // place
